...
<div class="content-entry">
	<div class="col-lg-12 col-lg-offset-1 bread-crumb">
		<ul>
			...
			if($idloai!=0){
				...
				?>
				<li> > <a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>0,'max1'=>0,'idtienich'=>0])}}"> {{$loaiks->Ten}} </a></li>
			<?php
			} 
			if($min1!=0 && $max1==0){ ?>
				<li> > <a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>$min,'max1'=>0,'idtienich'=>0])}}"> Dưới {{$min1}} </a></li>
				<?php
			}
			if($min1==0 && $max1!=0){
				?>
			<li> > <a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>0,'max1'=>$max,'idtienich'=>0])}}"> Trên {{$max}} </a></li>
				<?php
			}
			if($min1!=0 && $max1!=0){?>
				<li> > <a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>$min1,'max1'=>$max1,'idtienich'=>0])}}"> Từ {{$min1}} đến {{$max1}} </a></li>
		<?php
			}
			 ?>
		</ul>
	</div>
	<div class="col-lg-12 col-lg-offset-1"><h2> Khách sạn</h2></div>
	...
	<div class="panel panel-default">
		...
		<div class="col-lg-12"><a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>0,'min1'=>$min1,'max1'=>$max1,'idtienich'=>0])}}"> Tất cả</a></div>
		@foreach($loaikhachsan as $loai)
		<div class="col-lg-12"><a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$loai->id,'min1'=>$min1,'max1'=>$max1,'idtienich'=>0])}}"> {{ $loai->Ten}}</a></div>
		@endforeach
		</div>
	</div>
	<div class="panel panel-default">
		...
		<div class="col-lg-12"><a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>0,'max1'=>0,'idtienich'=>0])}}"> Tất cả</a></div>
			<div class="col-lg-12"><a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>$min,'max1'=>0,'idtienich'=>0])}}">Dưới {{ $min }}</a></div>
			<div class="col-lg-12"> <a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>$min,'max1'=>$array[0],'idtienich'=>0])}}">Từ {{ $min }} đến {{ $array[0] }}</a></div>
			@for($i=0;$i<(count($array)-1);$i++)
				<div class="col-lg-12"> <a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>$array[$i],'max1'=>$array[$i+1],'idtienich'=>0])}}">Từ {{ $array[$i] }} đến {{ $array[$i+1] }}</a></div>
			@endfor
			<div class="col-lg-12"><a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>0,'max1'=>$max,'idtienich'=>0])}}">Trên {{ $max }}</a></div>
		</div>
	</div>
	<div class="panel panel-default">
	...
	<div class="panel panel-default">
		...
		<div class="col-lg-12"><a href=""> Tất cả</a></div>
			@foreach($khuvucall as $all)
				<div class="col-lg-12"> <a href="">{{$all->TenKV}}</a></div>
			@endforeach
		</div>
	</div>
	</div>
...
?>
		<div class="panel panel-default" >
			...
			<a href="{{url('hotel/hotelall',['flag'=>$flag,'idloai'=>$idloai,'min1'=>$min1,'max1'=>$max1,'idtienich'=>0])}}"><button type="button">Giá <span class="glyphicon glyphicon-menu-down"></span></button></a>
			...
			@foreach($khachsan as $item)
				...
				<div class="hotelitem">
					...
					<div class="detail"><div style="margin-top: 30px;" class="price">{{$item->minprice}} VND</div><button type="button">Chi tiết</button></div>
					<div class="hotelinfo">
						<p class="hotelname"><a href="{{route('hotel.detail',['idks'=>$item->IDKhachSan])}}">{{$item->TenKhachSan}}</a></p>
